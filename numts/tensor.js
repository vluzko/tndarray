...
class tensor {
    ...
    slice(...indices) {
        ...
        const view = new tensor(this.data, new_shape.filter(filt), offset.filter(filt), new_stride, new_dstride, size, this.dtype, true, initial_offset);
        return view;
    }
    /**
    ...
    s(values, ...indices) {
        ...
        const iterator = utils_1.utils.zip_longest(view._iorder_data_iterator(), b_array._iorder_data_iterator());
     ...
     * @return {Iterable<number>}
     */
    _iorder_data_iterator(lower_or_upper, upper_bounds, steps) {
     ...
     * @private
     */
    _iorder_value_iterator(lower_or_upper, upper_bounds, steps) {
     ...
     * @private
     */
    _dorder_value_iterator(lower_or_upper, upper_bounds, steps) {
     ...
     * @private
     */
    static _upcast_to_tensor(value) {
        let a_array;
    ...
    static _broadcast_by_index(a, b) {
        ...
        let index_iter = indexing_1.indexing.iorder_index_iterator(new_dimensions);
        const iterator = utils_1.utils.zip_longest(a_array._iorder_data_iterator(), b_array._iorder_data_iterator(), index_iter);
        let iter = {};
    ...
    static broadcast_matmul(a, b) {
        ...
        if (new_dimensions.length === 2) {
            ...
            const index_iter = indexing_1.indexing.iorder_index_iterator(new_dimensions.slice(0, -2));
            const a_iter = indexing_1.indexing.iorder_index_iterator(a_shape.slice(0, -2));
            const b_iter = indexing_1.indexing.iorder_index_iterator(b_shape.slice(0, -2));
    ...
    static from_iterable(iterable, shape, dtype) {
        ...
        const array_type = utils_1.utils.dtype_map(dtype);
        const index_iterator = indexing_1.indexing.iorder_index_iterator(final_shape);
